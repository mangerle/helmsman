...
mod mock {
    ...
    /// 设置测试环境下的 PolicyKit 模拟权限
    ///
    /// 仅在启用 `test-support` feature 时编译，生产发行版构建中不存在该符号。
    ...
    /// 若测试 Mock 开关生效则返回应当放行的布尔值
    pub fn try_mock_override() -> Option<bool> {
        let mock_override = MOCK_POLKIT_OVERRIDE.load(Ordering::Relaxed);
        if mock_override < 0 {
            return None;
        }
        Some(mock_override == 1)
    }
}

#[cfg(feature = "test-support")]
pub use mock::set_mock_polkit_allow;
...
/// - **局限与权衡**：依赖宿主机运行有 PolicyKit 守护进程 (`polkitd`)；测试环境须启用 `test-support` feature 才能 Mock。
#[proxy(
...
pub async fn check_polkit_authorization(
    ...
    // 仅测试构建：内存 Mock 开关；生产 feature 关闭时此分支不存在
    #[cfg(feature = "test-support")]
    if let Some(allowed) = mock::try_mock_override() {
        debug!(
            "检测到 test-support Mock Polkit 覆盖值: {}，跳过系统 Polkit 校验",
            allowed
        );
        return Ok(allowed);
    }
